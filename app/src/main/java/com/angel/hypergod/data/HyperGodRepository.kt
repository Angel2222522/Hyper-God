...
class HyperGodRepository(private val context: Context) {
    ...
    suspend fun allDocumentEntities() = database.documentDao().getAll()
    ...
    suspend fun importUris(uris: List<Uri>): String? = withContext(Dispatchers.IO) {
        ...
        val documentsRoot = File(context.filesDir, "documents").apply {
            require(mkdirs() || isDirectory) { "Δεν ήταν δυνατή η δημιουργία του ιδιωτικού χώρου εγγράφων." }
        }
        val stagingDirectory = File(documentsRoot, ".import_$importToken").apply {
            require(mkdirs() || isDirectory) { "Δεν ήταν δυνατή η δημιουργία του ιδιωτικού χώρου εγγράφου." }
        }
        val pageDrafts = mutableListOf<DocumentPageEntity>()
        val sourceHashes = mutableListOf<String>()
        ...
        try {
            ...
            val contentHash = DocumentFingerprint.combineOrderedSourceHashes(sourceHashes)
            val id = DocumentFingerprint.newDocumentId(
                contentHash,
                UUID.randomUUID().toString().replace("-", "")
            )
            val documentDirectory = File(documentsRoot, id)
            require(!documentDirectory.exists()) { "Παρουσιάστηκε σύγκρουση αναγνωριστικού εγγράφου." }
            require(stagingDirectory.renameTo(documentDirectory)) {
                "Δεν ήταν δυνατή η οριστικοποίηση του ιδιωτικού χώρου εγγράφου."
            }
            installedDirectory = documentDirectory
            val pages = pageDrafts.map { page ->
                page.copy(
                    documentId = id,
                    encryptedPath = File(documentDirectory, File(page.encryptedPath).name).absolutePath
                )
            }
    ...
    suspend fun updateDocumentMetadata(
        ...
        runCatching { ReminderScheduler.replaceForDocument(context, id, reminderData.first, reminderData.second) }
            .onFailure { android.util.Log.w("HyperGod", "Document reminder reconciliation deferred", it) }
    }
    ...
    suspend fun createCase(
        ...
        runCatching { ReminderScheduler.replaceForCase(context, result.first, result.second, result.third) }
            .onFailure { android.util.Log.w("HyperGod", "Case reminder reconciliation deferred", it) }
        return result.first
    }
    ...
    suspend fun updateCase(
        ...
        DataOperationCoordinator.requireUserSessionUnlocked()
        // Status changes do not silently alter reminder semantics until the
        // product specifies whether
    ...
    suspend fun updateCaseStatus(id: String, status: String) = DataOperationCoordinator.withExclusive {
        DataOperationCoordinator.requireUserSessionUnlocked()
        // Keep COMPLETED reminder behavior unchanged until that policy is explicitly decided;
    ...
    /**
     * Removes only app-owned import generations that can be proven to have no
     * Room document. This repairs process death between the final file rename
     * and the database transaction without touching any referenced document.
     */
    suspend fun reconcileImportStorage() {
        DataOperationCoordinator.withExclusiveDuringStartup {
            val liveIds = database.documentDao().getAllIds().toSet()
            val root = context.filesDir.resolve("documents")
            root.listFiles().orEmpty()
                .filter { candidate ->
                    candidate.isDirectory &&
                        (candidate.name.startsWith(".import_") || candidate.name !in liveIds)
                }
                .forEach { orphan ->
                    require(FileCrypto.isPrivateDocumentFile(context, orphan)) {
                        "Μη ασφαλής διαδρομή προσωρινού εγγράφου."
                    }
                    FileCrypto.deleteRecursivelyStrict(orphan)
                }
        }
    }
