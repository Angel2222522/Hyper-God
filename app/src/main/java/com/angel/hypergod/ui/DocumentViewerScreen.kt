...
import com.angel.hypergod.data.DocumentPageDisplayPolicy
import com.angel.hypergod.data.DocumentRenderService
...
fun DocumentViewerScreen(document: DocumentEntity, onClose: () -> Unit) {
    ...
    var pages by remember(document.id, document.updatedAt) { mutableStateOf(emptyList<com.angel.hypergod.data.LogicalDocumentPage>()) }
    ...
    LaunchedEffect(document.id, document.updatedAt) {
        try {
            val loadedPages = withContext(Dispatchers.IO) { service.logicalPages(document) }
            pages = loadedPages
            pageIndex = 0
            error = null
